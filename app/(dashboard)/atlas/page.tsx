...
function Badge({ skor }: { skor: number }) {
  if (skor >= 75) return <span className="bg-green-900/50 text-green-400 border border-green-800 text-xs px-2 py-0.5 rounded-full">Sehat</span>
  if (skor >= 50) return <span className="bg-yellow-900/50 text-yellow-400 border border-yellow-800 text-xs px-2 py-0.5 rounded-full">Cukup</span>
  return <span className="bg-red-900/50 text-red-400 border border-red-800 text-xs px-2 py-0.5 rounded-full">Perhatian</span>
}
...
export default function AtlasPage() {
  ...
  useEffect(() => {
    supabase.from('atlas_agregat').select('*')
      .then(({ data: rows, error }) => {
        if (error) {
          // Fallback: hitung manual kalau VIEW belum dibuat
          loadManual()
          return
        }
  ...
  async function loadManual() {
    ...
    const rows: AtasRow[] = await Promise.all(kops.map(async k => {
      ...
      return {
        koperasi_id: k.id, nama: k.nama,
        jumlah_anggota: anggota ?? 0,
        total_simpanan: totalSimpanan,
        jumlah_ternak: ternak?.length ?? 0,
        pct_sehat: pctSehat,
        pinjaman_aktif: pinjaman ?? 0,
        skor,
      }
    }))
  ...
  function SortIcon({ k }: { k: keyof AtasRow }) {
    ...
    return <span className="text-green-400">{sortAsc ? '↑' : '↓'}</span>
  }

  return (
    <div className="max-w-5xl mx-auto space-y-5">
      <div>
        <h1 className="text-white text-xl font-semibold">Lumbung Atlas</h1>
        <p className="text-slate-400 text-sm">Ringkasan agregat semua koperasi — Dinas Koperasi Kabupaten</p>
      </div>

      {/* Banner privasi */}
      <div className="bg-blue-950/40 border border-blue-800/50 rounded-xl px-4 py-3 flex items-center gap-2 text-xs text-blue-300">
        <span>🔒</span>
      ...
      {!loading && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Koperasi Binaan', value: data.length, color: 'text-white' },
            { label: 'Total Simpanan Platform', value: `Rp ${(totalSimpanan / 1_000_000).toFixed(1)}jt`, color: 'text-green-400' },
            { label: 'Rata-rata Skor', value: avgSkor, color: avgSkor >= 75 ? 'text-green-400' : avgSkor >= 50 ? 'text-yellow-400' : 'text-red-400' },
          ].map(s => (
      ...
      {/* Tabel komparasi */}
      {loading ? (
        <p className="text-slate-500 text-sm">Memuat data agregat...</p>
      ) : (
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-800">
              {sorted.map(row => (
                <tr key={row.koperasi_id} className="hover:bg-slate-800/50 transition-colors">
                  <td className="px-4 py-3 text-white font-medium">{row.nama}</td>
                  <td className="px-4 py-3 text-slate-300">{row.jumlah_anggota}</td>
                  <td className="px-4 py-3 text-slate-300">
                    Rp {(row.total_simpanan / 1_000_000).toFixed(1)}jt
                  </td>
                  <td className="px-4 py-3 text-slate-300">{row.jumlah_ternak}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div className="h-full bg-green-500/70 rounded-full"
                          style={{ width: `${row.pct_sehat}%` }} />
                      </div>
                      <span className="text-slate-300 text-xs">{row.pct_sehat}%</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-300">{row.pinjaman_aktif}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${row.skor >= 75 ? 'text-green-400' : row.skor >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                        {row.skor}
                      </span>
                      <Badge skor={row.skor} />
                    </div>
      ...
      {/* Bar chart visual simpel */}
      {!loading && data.length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <p className="text-slate-300 text-sm font-medium mb-4">Perbandingan Total Simpanan</p>
            ...
            {[...data].sort((a, b) => b.total_simpanan - a.total_simpanan).map(row => {
              ...
              return (
                <div key={row.koperasi_id} className="flex items-center gap-3">
                  <span className="text-slate-400 text-xs w-32 truncate">{row.nama}</span>
                  <div className="flex-1 h-5 bg-slate-800 rounded overflow-hidden">
                    <div className="h-full bg-green-600/70 rounded transition-all flex items-center px-2"
                      style={{ width: `${pct}%` }}>
                      {pct > 20 && (
                  ...
                  {pct <= 20 && (
                    <span className="text-slate-400 text-xs">Rp {(row.total_simpanan / 1_000_000).toFixed(1)}jt</span>
                  )}
                </div>
              )
            })}
